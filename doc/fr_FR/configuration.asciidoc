...
- serrure : les serrures sont créées automatiquement par la configuration générale du plugin
  ...
  deux actions verrouillage et déverouillage sont créées pour chaque gateway ajoutée

- gateway : la gateway est un équipement ajouté manuellement dans le plugin avec son IP et son ID (se référer pour ce dernier à l'étiquette The Keys)
